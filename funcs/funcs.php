<?php
	...
	function emailExiste($email)
	{
		...
		$stmt = $mysqli->prepare("SELECT idUsuario FROM profesores WHERE correo = ? LIMIT 1");
	...
	function enviarEmail($email, $nombre, $asunto, $cuerpo){
		...
		$mail->Username = 'cfunchona@gmail.com';
		$mail->Password = 'Contrasena.2'; 
		
		$mail->setFrom('cfunchona@gmail.com', 'CANCEI-ESCOM'); 
	...
	function cambiaPassword($password, $user_id, $token){
		...
		$stmt = $mysqli->prepare("UPDATE usuario SET contrasenia = ?, token_password='', password_request=0 WHERE idUsuario = ? AND token_password = ?");
